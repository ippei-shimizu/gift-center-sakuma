<footer class="footer">
  <div class="footer-inner">
    <div class="footer-left">
      <a href="<?php echo esc_url(home_url('/')); ?>" class="footer-logo">
        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/footer-logo.png" width="132px" height="132px" alt="ギフトセンターサクマ" loading="lazy">
      </a>
      <h2 class="footer-title">ギフトセンターサクマ</h2>
      <div class="footer-info">
        <p><span class="address">〒409-3866</span><br class="sp-br">山梨県中巨摩郡昭和町西条3812-2</p>
        <a href="tel:055-275-5178">TEL.055-275-5178</a>
      </div>
      <p class="google-map">
        <a href="https://www.google.com/maps/place/%E3%82%AE%E3%83%95%E3%83%88%E3%82%BB%E3%83%B3%E3%82%BF%E3%83%BC%E3%82%B5%E3%82%AF%E3%83%9E/@35.642432,138.5417081,17z/data=!3m1!4b1!4m6!3m5!1s0x601bf769fca1ed9f:0x6e1afe211f1f783e!8m2!3d35.642432!4d138.544283!16s%2Fg%2F1thkxbfk?hl=ja&entry=ttu" target="_blank">
        Google map
        </a>
      </p>
      <p class="footer-time">
      営業時間　9:30～19:00<br>
      定休日　水曜日
      </p>
      <div class="footer-in">
        <p>T7810154416408</p>
        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/invoice.png" width="40px" height="40px" alt="インボイス登録店" loading="lazy">
      </div>
    </div>
    <div class="footer-right">
    <div class="nav-contents">
        <div class="nav-contents-title">
          <p>MENU</p>
        </div>
        <div class="nav-border"></div>
        <div class="nav-contents-box nav-contents-box-01">
          <p class="nav-list-title">ギフトの選び方・贈り方</p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">結婚</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">出産</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">快気・お見舞い</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">入進学・就職・長寿</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">新築・引越し</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">仏事・法要</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">御中元・御歳暮</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">イベント景品</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/how-to-give')); ?>/">カタログギフト</a>
          </p>
        </div>
        <div class="nav-border"></div>
        <div class="nav-contents-box">
          <p class="nav-list-title">オリジナルサービス</p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">カタログギフト</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">地元ギフト</a>
          </p>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">仏事</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">御中元・御歳暮</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">配送</a>
          </p>
          <p class="nav-list-title"><a href="<?php echo esc_url(home_url('/')); ?>/">ABOUT</a></p>
          <p class="nav-list-title">TOPIX</p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">おしらせ</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">Q&A</a>
          </p>
          <p class="nav-list-link">
            <a href="<?php echo esc_url(home_url('/')); ?>/">PICK UP</a>
          </p>
        </div>
        <p class="copy">&copy;giftcenter Sakuma</p>
      </div>
    </div>
    <p class="copy-sp">&copy;giftcenter Sakuma</p>
  </div>
  <div id="pageTop" class="page-top">
    <img src="<?php echo get_template_directory_uri(); ?>/assets/images/pagetop.svg" width="20px" height="29px" alt="ページトップ" loading="lazy">
  </ぢ>
</footer>

<script src="https://cdnjs.cloudflare.com/ajax/libs/Swiper/8.4.2/swiper-bundle.js" integrity="sha512-rBoFCxH4xQ7PyI9atv97YuFNLXBf4vKJoA3578CMugbM01FbEwJ68fe3hhL3aKRlqZHlxCtlagXIcQ0GZJyhvw==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>
<script src="https://cdn.jsdelivr.net/npm/gsap@3.12/dist/gsap.min.js"></script>
<script src="//cdn.jsdelivr.net/npm/gsap@3.7.0/dist/ScrollTrigger.min.js"></script>
<?php wp_footer(); ?>
</body>
</html>